...
___brainy_prompt_aws() {
	[ "${THEME_SHOW_AWS}" != "true" ] && return
	color=$bold_yellow
	box="[|]"
	info="$(aws_prompt)"
	printf "%s|%s|%s|%s" "${color}" "${info}" "${bold_blue}" "${box}"
...
THEME_SHOW_AWS=${THEME_SHOW_AWS:-"true"}
...
___BRAINY_TOP_RIGHT=${___BRAINY_TOP_RIGHT:-"python aws ruby todo clock battery"}
...
python_version_prompt() {
	if [[ -n "$VIRTUAL_ENV" ]]; then
		echo -e "venv: $(py_interp_prompt)"
	else
		echo -e "$(py_interp_prompt)"
	fi
}
